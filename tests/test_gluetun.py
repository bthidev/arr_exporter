...
@responses.activate
def test_collect_builds_points_from_openvpn(settings):
    ...
    assert point._tags["protocol"] == "openvpn"
...
@responses.activate
def test_collect_falls_back_to_wireguard(settings):
    responses.add(responses.GET, f"{BASE}/openvpn/status", status=404)
    responses.add(responses.GET, f"{BASE}/wireguard/status", json={"status": "running"})
    responses.add(responses.GET, f"{BASE}/publicip/ip", json=load_fixture("gluetun_publicip.json"))

    points = gluetun.collect(settings)

    assert points[0]._tags["protocol"] == "wireguard"


@responses.activate
def test_collect_reports_tunnel_down(settings):
    responses.add(responses.GET, f"{BASE}/openvpn/status", json={"status": "stopped"})
    responses.add(responses.GET, f"{BASE}/publicip/ip", json=load_fixture("gluetun_publicip.json"))
